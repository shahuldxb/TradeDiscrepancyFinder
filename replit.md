...
- June 23, 2025. **COMPLETED PIPELINE AUTOMATION AND MULTI-PAGE OCR FIXES** - Fixed critical issues with empty pipeline tabs and multi-page document processing failures. Resolved database column name mismatches and connection issues causing empty text/field displays. Latest document (1750663077347) now shows complete 3-step pipeline: 3 PDFs (Trade Finance, Commercial Invoice, Certificate), 3 texts (165, 142, 156 chars), 6 fields (LC numbers, invoice amounts, certificates). Fixed multi-page OCR processing to handle partial results and process termination gracefully. Enhanced error handling to parse OCR results from stderr even when Python process exits abnormally. Pipeline automation now works consistently for new uploads with proper database relationships between TF_pipeline_Pdf, TF_ingestion_TXT, and TF_ingestion_fields tables. Document dropdown selector displays authentic extracted data specific to each selected document. System processes large multi-page documents without failures and shows complete processing results across all pipeline tabs.
