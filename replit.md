...
- June 27, 2025. **COMPLETED LARGE DOCUMENT OCR CRASH FIX** - Successfully resolved 38-page document OCR failures that were crashing at page 18-27 by completely replacing OpenCV OCR with memory-efficient processing. Fixed all remaining references to opencvOCR.py throughout routes.ts to use memoryEfficientOCR.py exclusively. System now processes large documents (30+ pages) without null exit code crashes, memory exhaustion, or page-specific failures. Enhanced error handling for partial results parsing when Python process exits unexpectedly. Tested with ILCAE SWIFT document (3 forms extracted) and Certificate of Origin (2 forms extracted) - both complete successfully with authentic text extraction and automatic pipeline execution. Memory-efficient OCR now standard processing method with 10-minute timeout for large documents.
