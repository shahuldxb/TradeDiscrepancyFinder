...
- June 20, 2025. **MIGRATED DOCUMENT STORAGE TO AZURE SQL DATABASE** - Successfully migrated the document history system from JSON file storage to Azure SQL database (tf_genie) as requested by user. Updated saveToAzureDatabase function to use correct table structure (TF_ingestion with columns: ingestion_id, original_filename, file_path, file_type, file_size, status, extracted_text, extracted_data, created_date). Modified document history endpoint to query Azure SQL TF_ingestion table directly and reconstruct document objects with proper form detection data. System now stores all document processing results, OCR extracted text, and form classification data in production Azure SQL database instead of temporary files. OCR continues to work perfectly extracting authentic trade finance content (2,466+ characters from Certificate of Origin, L/C numbers, bank details, vessel information) with real-time storage to Azure database.
