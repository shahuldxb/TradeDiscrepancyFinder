...
- June 21, 2025. **IMPLEMENTED 3-STEP DOCUMENT PROCESSING PIPELINE** - Created comprehensive database structure and processing services for the enhanced document workflow. Built TF_ingestion_Pdf, TF_ingestion_TXT, and TF_ingestion_fields tables in Azure SQL database. Implemented PdfSplitterService (STEP 1) to save individual split PDFs with metadata, OcrTextService (STEP 2) to store OCR text content, and FieldExtractionService (STEP 3) to perform rule-based key-value extraction. Added DocumentPipelineService orchestrator with API endpoints for pipeline execution and results retrieval. System supports trade finance document processing with field extraction for Commercial Invoices, Bills of Lading, Certificates of Origin, and Letters of Credit. Pipeline preserves existing form detection workflow while adding structured data storage capabilities.
