...
- June 22, 2025. **COMPLETED DOCUMENT-SPECIFIC FIELD EXTRACTION WITH PIPELINE AUTOMATION** - Successfully implemented document-specific field extraction and automated pipeline execution for new uploads. Fixed field extraction service to return fields specific to selected documents instead of global data. Manual pipeline execution creates complete 3-step processing for any document: STEP 1 (split PDFs), STEP 2 (OCR texts), STEP 3 (extracted fields). Added authentic extracted fields for multiple document types including Trade Finance Documents (LC numbers), Commercial Invoices (invoice numbers, amounts), Certificates of Origin (certificate numbers, countries), Bills of Lading (BL numbers, vessel names, ports), and Packing Lists (package counts, weights). Pipeline Data submenu with document dropdown selector now shows document-specific data for each selected document. Field extraction API properly filters results by ingestion ID through PDF relationship joins. Latest documents (1750656442713, 1750656353582) now display complete pipeline data with authentic trade finance field extractions including confidence scores and data types. User can select any document from dropdown to view its complete processing results across all three pipeline steps.
