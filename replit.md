...
- June 21, 2025. **COMPLETED 3-STEP PIPELINE WITH UI SUBMENU AND API INTEGRATION** - Successfully implemented complete 3-step document processing pipeline with Azure SQL integration and new UI submenu. Pipeline processes documents through: STEP 1 (saves split PDFs to TF_pipeline_Pdf), STEP 2 (stores OCR text to TF_ingestion_TXT), STEP 3 (extracts fields to TF_ingestion_fields). Manual pipeline execution fully operational via POST /api/document-pipeline/execute endpoint. Added new "Pipeline Data" submenu under Form Detection with three tabs (PDF, Text, Fields) displaying real pipeline data from Azure SQL. Created PipelineDataView.tsx component with comprehensive data display including confidence scores, text content, extracted fields, and processing metadata. Added pipeline API endpoints (/api/pipeline/pdfs, /api/pipeline/texts, /api/pipeline/fields) for UI data fetching. Fixed automatic pipeline integration in upload workflow. Pipeline tables populated with authentic trade finance data from multiple document types. User can now view complete pipeline processing results through dedicated UI interface.
