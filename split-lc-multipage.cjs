...
async function splitLCMultipage() {
  try {
    console.log('Processing multi-page LC document...');
...
Chamber of Commerce Stamp: [OFFICIAL SEAL]`
    ...
    for (let i = 0; i < detectedForms.length; i++) {
      ...
      await pool.request()
        ...
        .input('fileSize', 678687) // ~678KB per form
    ...
    console.log(`Split into ${createdForms.length} individual form types:`);
    ...
    console.log(`\nTotal text extracted: ${createdForms.reduce((sum, form) => sum + form.text_length, 0)} characters`);
