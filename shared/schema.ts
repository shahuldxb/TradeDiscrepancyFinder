...
// MT Genie Enhanced Schema - Comprehensive MT Message Management System
export const mtGenieMessageTypes = pgTable("mt_genie_message_types", {
  id: varchar("id").primaryKey(),
  messageTypeCode: varchar("message_type_code").notNull().unique(),
  name: varchar("name").notNull(),
  fullName: varchar("full_name").notNull(),
  purpose: text("purpose").notNull(),
  signed: boolean("signed").notNull(),
  maxLength: integer("max_length").notNull(),
  mug: boolean("mug").notNull(),
  category: varchar("category", { length: 2 }).notNull().default("7"),
  scope: text("scope"),
  formatSpecifications: text("format_specifications"),
  networkValidatedRules: jsonb("network_validated_rules"),
  usageRules: jsonb("usage_rules"),
  isActive: boolean("is_active").default(true),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

export const mtGenieMessageFields = pgTable("mt_genie_message_fields", {
  id: varchar("id").primaryKey(),
  messageTypeId: varchar("message_type_id").references(() => mtGenieMessageTypes.id),
  tag: varchar("tag").notNull(),
  fieldName: varchar("field_name").notNull(),
  isMandatory: boolean("is_mandatory").notNull(),
  contentOptions: varchar("content_options"),
  sequence: integer("sequence").notNull(),
  formatDescription: text("format_description"),
  presenceDescription: text("presence_description"),
  definitionDescription: text("definition_description"),
  validationRules: jsonb("validation_rules"),
  formatOptions: jsonb("format_options"),
  fieldCodes: jsonb("field_codes"),
...
export const mtGenieMessageDependencies = pgTable("mt_genie_message_dependencies", {
  id: varchar("id").primaryKey(),
  sourceMessageTypeId: varchar("source_message_type_id").references(() => mtGenieMessageTypes.id),
  targetMessageTypeId: varchar("target_message_type_id").references(() => mtGenieMessageTypes.id),
  dependencyType: varchar("dependency_type").notNull(),
  description: text("description"),
  isActive: boolean("is_active").default(true),
  createdAt: timestamp("created_at").defaultNow(),
});

export const mtGenieMessageInstances = pgTable("mt_genie_message_instances", {
  id: varchar("id").primaryKey(),
  messageTypeId: varchar("message_type_id").references(() => mtGenieMessageTypes.id),
...
export const mtGenieFieldValues = pgTable("mt_genie_field_values", {
  id: varchar("id").primaryKey(),
  instanceId: varchar("instance_id").references(() => mtGenieMessageInstances.id),
  fieldId: varchar("field_id").references(() => mtGenieMessageFields.id),
...
export const mtGenieTemplates = pgTable("mt_genie_templates", {
  id: varchar("id").primaryKey(),
  messageTypeId: varchar("message_type_id").references(() => mtGenieMessageTypes.id),
...
export const mtGenieWorkflows = pgTable("mt_genie_workflows", {
  id: varchar("id").primaryKey(),
  workflowName: varchar("workflow_name").notNull(),
  description: text("description"),
  messageTypeSequence: jsonb("message_type_sequence"),
...
// Create insert schemas for MT Genie
export const insertMTGenieMessageTypeSchema = createInsertSchema(mtGenieMessageTypes).pick({
  messageTypeCode: true,
  name: true,
  fullName: true,
  purpose: true,
  ...
  category: true,
  scope: true,
  formatSpecifications: true,
  networkValidatedRules: true,
  usageRules: true,
});

export const insertMTGenieMessageFieldSchema = createInsertSchema(mtGenieMessageFields).pick({
  tag: true,
  fieldName: true,
  isMandatory: true,
  contentOptions: true,
...
export const insertMTGenieMessageInstanceSchema = createInsertSchema(mtGenieMessageInstances).pick({
...
export const insertMTGenieTemplateSchema = createInsertSchema(mtGenieTemplates).pick({
  templateName: true,
  description: true,
  templateData: true,
  isPublic: true,
...
export const insertMTGenieWorkflowSchema = createInsertSchema(mtGenieWorkflows).pick({
  workflowName: true,
  description: true,
  messageTypeSequence: true,
  workflowSteps: true,
});
...
// MT Genie Types
export type MTGenieMessageType = typeof mtGenieMessageTypes.$inferSelect;
export type InsertMTGenieMessageType = z.infer<typeof insertMTGenieMessageTypeSchema>;
export type MTGenieMessageField = typeof mtGenieMessageFields.$inferSelect;
export type InsertMTGenieMessageField = z.infer<typeof insertMTGenieMessageFieldSchema>;
export type MTGenieMessageInstance = typeof mtGenieMessageInstances.$inferSelect;
export type InsertMTGenieMessageInstance = z.infer<typeof insertMTGenieMessageInstanceSchema>;
export type MTGenieTemplate = typeof mtGenieTemplates.$inferSelect;
export type InsertMTGenieTemplate = z.infer<typeof insertMTGenieTemplateSchema>;
export type MTGenieWorkflow = typeof mtGenieWorkflows.$inferSelect;
export type InsertMTGenieWorkflow = z.infer<typeof insertMTGenieWorkflowSchema>;

// MT Intelligence Tables
export const mtIntelligenceMessageTypes = pgTable("mt_intelligence_message_types", {
  id: varchar("id").primaryKey(),
  messageTypeCode: varchar("message_type_code").notNull(),
  name: varchar("name").notNull(),
  description: text("description"),
...
export const mtIntelligenceFields = pgTable("mt_intelligence_fields", {
  id: varchar("id").primaryKey(),
...
export const mtIntelligenceMessageTypeFields = pgTable("mt_intelligence_message_type_fields", {
  id: varchar("id").primaryKey(),
  messageTypeId: varchar("message_type_id").references(() => mtIntelligenceMessageTypes.id),
  fieldId: varchar("field_id").references(() => mtIntelligenceFields.id),
  sequence: integer("sequence").notNull(),
...
export const mtIntelligenceFieldDependencies = pgTable("mt_intelligence_field_dependencies", {
  id: varchar("id").primaryKey(),
  sourceFieldId: varchar("source_field_id").references(() => mtIntelligenceFields.id),
  targetFieldId: varchar("target_field_id").references(() => mtIntelligenceFields.id),
  dependencyType: varchar("dependency_type").notNull(), // Requires, Excludes, Implies
  conditionExpression: text("condition_expression"),
  messageTypeId: varchar("message_type_id").references(() => mtIntelligenceMessageTypes.id),
  description: text("description"),
  isActive: boolean("is_active").default(true),
...
export const mtIntelligenceValidationRules = pgTable("mt_intelligence_validation_rules", {
  id: varchar("id").primaryKey(),
  ...
  fieldId: varchar("field_id").references(() => mtIntelligenceFields.id),
  messageTypeId: varchar("message_type_id").references(() => mtIntelligenceMessageTypes.id),
  ruleType: varchar("rule_type").notNull(), // Format, Content, Dependency, Business
  ruleExpression: text("rule_expression").notNull(),
  errorMessage: text("error_message").notNull(),
  severity: varchar("severity").notNull(), // error, warning, info
  ucpReference: varchar("ucp_reference"),
  isActive: boolean("is_active").default(true),
  createdAt: timestamp("created_at").defaultNow(),
});

export const mtIntelligenceMessages = pgTable("mt_intelligence_messages", {
  id: varchar("id").primaryKey(),
  messageTypeId: varchar("message_type_id").references(() => mtIntelligenceMessageTypes.id),
  referenceNumber: varchar("reference_number").notNull(),
  content: text("content").notNull(),
  status: varchar("status").notNull(), // draft, validated, sent, error
...
export const mtIntelligenceValidationResults = pgTable("mt_intelligence_validation_results", {
  id: varchar("id").primaryKey(),
  messageId: varchar("message_id").references(() => mtIntelligenceMessages.id),
...
export const mtIntelligenceValidationErrors = pgTable("mt_intelligence_validation_errors", {
  id: varchar("id").primaryKey(),
  resultId: varchar("result_id").references(() => mtIntelligenceValidationResults.id),
  fieldId: varchar("field_id").references(() => mtIntelligenceFields.id),
  ...
  ruleId: varchar("rule_id").references(() => mtIntelligenceValidationRules.id),
});

// MT Intelligence Zod schemas
export const insertMtIntelligenceMessageTypeSchema = createInsertSchema(mtIntelligenceMessageTypes).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertMtIntelligenceFieldSchema = createInsertSchema(mtIntelligenceFields).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertMtIntelligenceMessageSchema = createInsertSchema(mtIntelligenceMessages).omit({
  id: true,
  createdAt: true,
  lastModifiedAt: true,
});

export const insertMtIntelligenceValidationResultSchema = createInsertSchema(mtIntelligenceValidationResults).omit({
  id: true,
  validationDate: true,
});

// Export types for MT Intelligence
export type MtIntelligenceMessageType = typeof mtIntelligenceMessageTypes.$inferSelect;
export type InsertMtIntelligenceMessageType = z.infer<typeof insertMtIntelligenceMessageTypeSchema>;

export type MtIntelligenceField = typeof mtIntelligenceFields.$inferSelect;
export type InsertMtIntelligenceField = z.infer<typeof insertMtIntelligenceFieldSchema>;

export type MtIntelligenceMessage = typeof mtIntelligenceMessages.$inferSelect;
export type InsertMtIntelligenceMessage = z.infer<typeof insertMtIntelligenceMessageSchema>;

export type MtIntelligenceValidationResult = typeof mtIntelligenceValidationResults.$inferSelect;
export type InsertMtIntelligenceValidationResult = z.infer<typeof insertMtIntelligenceValidationResultSchema>;