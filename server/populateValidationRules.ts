...
function parseSwiftFormat(format: string, fieldId: number, messageTypeId: number, fieldTag: string, fieldName: string, isMandatory: boolean) {
  const rules: any[] = [];
...
export async function populateValidationRules() {
  try {
...
async function insertValidationRulesBatch(pool: any, rules: any[]) {
