...
Generated by Trade Finance Forms Recognition System`;
  ...
  app.post('/api/forms/populate-processing-tables', async (req, res) => {
    try {
      ...
      // If tables are already populated, return current status
      if (pdfCount > 0 && txtCount > 0) {
        res.json({
          success: true,
          message: `Processing tables already populated. ${statusMessage.join(', ')}`,
          pdfCount,
          txtCount,
      ...
      // Try to populate additional records if needed
      if (pdfCount === 0) {
        const newPdfFiles = await pool.request().query(`
          SELECT TOP 5
            ingestion_id,
            original_filename,
            document_type,
        ...
        for (const pdf of newPdfFiles.recordset) {
          try {
            await pool.request()
            ...
            console.log(`PDF record ${pdf.ingestion_id} already exists or constraint issue`);
          }
        }
        statusMessage.push(`Added ${pdfCount} new PDF records`);
      }
      
      res.json({
        success: true,
        message: `Processing tables populated successfully. ${statusMessage.join(', ')}`,
        pdfCount,
        txtCount,
        status: 'completed'
      });
