...
Generated by Trade Finance Forms Recognition System`;
  ...
  app.post('/api/forms/python-upload', upload.single('file'), async (req, res) => {
    try {
      ...
      await pool.request()
        ...
        .query(`
          INSERT INTO TF_ingestion (
            ingestion_id, original_filename, file_path, file_size, mime_type, status
          ) VALUES (
            @ingestionId, @originalFilename, @filePath, @fileSize, @mimeType, @status
          )
        `);
  ...
  app.get('/api/forms/processing-status/:ingestionId', async (req, res) => {
    try {
      ...
      if (result.recordset.length === 0) {
        return res.status(404).json({ completed: false, error: 'Record not found' });
      }
      ...
      res.json({
        completed,
        stage: record.status,
        currentAction: record.status === 'completed' ? 'Processing completed successfully' :
                      record.status === 'processing' ? 'Analyzing forms with Azure Document Intelligence' :
                      record.status === 'uploaded' ? 'File uploaded, ready for processing' : 'Processing...',
      ...
      res.status(500).json({ completed: false, error: 'Status check failed' });
    }
  });
