...
Generated by Trade Finance Forms Recognition System`;
  ...
  // Get form details for enhanced processing
  app.get('/api/forms/form-details/:ingestionId', async (req, res) => {
    try {
      ...
      const ingestion = mainRecord.recordset[0];
      
      // Create form details from available data
      const forms = [{
        index: 1,
        pdfData: {
          filename: ingestion.original_filename,
          size: ingestion.file_size || 0,
          type: ingestion.mime_type || 'application/pdf',
          path: ingestion.file_path
        },
        textData: {
          content: ingestion.extracted_text || '',
          characterCount: ingestion.extracted_text ? ingestion.extracted_text.length : 0,
          wordCount: ingestion.extracted_text ? ingestion.extracted_text.split(/\s+/).length : 0
        },
        fieldsData: JSON.parse(ingestion.extracted_data || '{}'),
        formType: ingestion.document_type || 'Unknown',
        confidence: 85.5,
        extractedFields: JSON.parse(ingestion.extracted_data || '{}'),
        hasText: !!ingestion.extracted_text,
        hasPdf: true,
        hasFields: !!ingestion.extracted_data
      }];
