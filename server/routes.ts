  ...
  app.get('/api/forms/download/:ingestionId/:type', async (req, res) => {
    try {
      ...
      if (type === 'text') {
        ...
        // Get original PDF file path from TF_ingestion table
        const pdfQuery = `
  ...
  app.get('/api/forms/processing-status/:ingestionId', async (req, res) => {
    try {
      ...
      const statusQuery = `
        SELECT 
          i.*,
          COUNT(p.id) as pdf_records_count,
          COUNT(t.id) as txt_records_count,
          COUNT(f.id) as field_records_count
        FROM TF_ingestion i
        ...
        LEFT JOIN TF_ingestion_fields f ON i.ingestion_id = f.ingestion_id
        WHERE i.ingestion_id = @ingestionId
        GROUP BY i.id, i.ingestion_id, i.file_path, i.file_type, i.original_filename, 
                 i.file_size, i.status, i.created_date, i.updated_date, i.processing_steps,
                 i.document_type, i.total_forms_detected, i.classification_confidence,
                 i.error_message, i.processing_time_ms
      `;
