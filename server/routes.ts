...
Generated by Trade Finance Forms Recognition System`;
  ...
  // Forms Recognizer - Upload and process multi-form PDF with Python backend
  app.post('/api/forms/python-upload', (req, res) => {
    upload.single('file')(req, res, async (err) => {
      ...
      try {
        ...
        if (!req.file.buffer) {
          console.log('File buffer is missing - multer configuration issue');
          return res.status(400).json({ 
            success: false, 
            message: 'File buffer missing - upload configuration error' 
          });
        }
        ...
        // Save file and create ingestion record
        const { filePath, ingestionId } = await service.saveFileAndCreateIngestion(req.file);
      
      // Create initial ingestion record in database
      const { connectToAzureSQL } = await import('./azureSqlConnection');
      const pool = await connectToAzureSQL();
      
      // First, let's check what columns actually exist in the table
      const tableInfo = await pool.request()
        .query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'TF_ingestion'");
      
      console.log('TF_ingestion table columns:', tableInfo.recordset.map(r => r.COLUMN_NAME));
      
      // Insert without mime_type column for now to avoid the error
      await pool.request()
        .input('ingestionId', ingestionId)
        ...
        .query(`
          INSERT INTO TF_ingestion (
            ingestion_id, original_filename, file_path, file_size, status
          ) VALUES (
            @ingestionId, @originalFilename, @filePath, @fileSize, @status
          )
        `);
