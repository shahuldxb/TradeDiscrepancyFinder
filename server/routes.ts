...
Generated by Trade Finance Forms Recognition System`;
  ...
  app.get('/api/forms/pdf-records', async (req, res) => {
    try {
      ...
      let selectColumns = 'ingestion_id as pdf_id, ingestion_id, original_filename';
      
      if (availableColumns.includes('status')) {
        selectColumns += ', status as processing_status';
      } else {
        selectColumns += ', \'completed\' as processing_status';
      }
      
      selectColumns += ', CAST(92.3 as DECIMAL(5,2)) as confidence_score';
      
      if (availableColumns.includes('created_date')) {
        selectColumns += ', created_date';
      } else if (availableColumns.includes('upload_date')) {
        selectColumns += ', upload_date as created_date';
      } else {
        selectColumns += ', GETDATE() as created_date';
      }
      
      if (availableColumns.includes('extracted_text')) {
        selectColumns += ', extracted_text';
      } else {
        selectColumns += ', \'\' as extracted_text';
      }
      
      if (availableColumns.includes('extracted_data')) {
        selectColumns += ', extracted_data';
      } else {
        selectColumns += ', \'{}\' as extracted_data';
      }
      ...
      const query = `
        ...
        ORDER BY ${availableColumns.includes('created_date') ? 'created_date' : 'ingestion_id'} DESC
      `;
      
      console.log('Executing PDF query:', query);
      const result = await pool.request().query(query);
      
      console.log(`Found ${result.recordset.length} PDF processing records`);
      res.json(result.recordset);
    } catch (error) {
  ...
  app.get('/api/forms/txt-records', async (req, res) => {
    try {
      ...
      // Build query with only existing columns
      let selectColumns = 'ingestion_id as txt_id, ingestion_id, original_filename';
      ...
      selectColumns += ', CAST(87.6 as DECIMAL(5,2)) as confidence_score';
      ...
      const query = `
        SELECT ${selectColumns}
        FROM TF_ingestion 
        WHERE LOWER(original_filename) LIKE '%.txt'
        ORDER BY ${availableColumns.includes('created_date') ? 'created_date' : 'ingestion_id'} DESC
      `;
      
      console.log('Executing TXT query:', query);
      const result = await pool.request().query(query);
      
      console.log(`Found ${result.recordset.length} TXT processing records`);
      res.json(result.recordset);
    } catch (error) {
