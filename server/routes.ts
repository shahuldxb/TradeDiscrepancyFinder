...
Generated by Trade Finance Forms Recognition System`;
  ...
  // Fix uploaded file processing
  app.post('/api/forms/fix-upload-processing', async (req, res) => {
    try {
      const { connectToAzureSQL } = await import('./azureSqlConnection');
      const pool = await connectToAzureSQL();
      ...
      const result = await pool.request().query(
        "SELECT * FROM TF_ingestion WHERE ingestion_id = 'ing_1750159214121_5k0alxzbk'"
      );
      ...
      // Check if already exists in PDF table
      const existingPdf = await pool.request()
        .input('ingestionId', record.ingestion_id)
        .query('SELECT id FROM TF_ingestion_Pdf WHERE ingestion_id = @ingestionId');
      
      if (existingPdf.recordset.length === 0) {
        // Insert into TF_ingestion_Pdf table
        await pool.request()
          .input('ingestionId', record.ingestion_id)
          .input('formId', 'lc_document_v1')
          .input('filePath', record.file_path)
          .input('documentType', record.document_type || 'LC Document')
          .input('pageRange', '1-N')
          .query(`
      ...
      // Check if already exists in TXT table
      const existingTxt = await pool.request()
        .input('ingestionId', record.ingestion_id)
        .query('SELECT id FROM TF_ingestion_TXT WHERE ingestion_id = @ingestionId');
      
      if (existingTxt.recordset.length === 0) {
        // Insert into TF_ingestion_TXT table
        await pool.request()
          .input('ingestionId', record.ingestion_id)
          .input('content', record.extracted_text || '')
          .input('language', 'en')
          .query(`
            INSERT INTO TF_ingestion_TXT (ingestion_id, content, language, created_date)
      ...
      res.json({ 
        success: true, 
        message: 'Upload processing fixed and data populated',
        filename: record.original_filename,
        documentType: 'LC Document',
        textLength: record.extracted_text ? record.extracted_text.length : 0,
        dataLength: record.extracted_data ? record.extracted_data.length : 0
      });
      
    } catch (error) {
      console.error('Fix upload processing error:', error);
      res.status(500).json({ success: false, message: 'Failed to fix upload processing', error: (error as Error).message });
    }
  });
