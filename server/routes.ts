...
Generated by Trade Finance Forms Recognition System`;
  ...
  // Group documents by form types
  app.get('/api/forms/grouped-documents', async (req, res) => {
    try {
      const { connectToAzureSQL } = await import('./azureSqlConnection');
      const pool = await connectToAzureSQL();
      
      // Get all form definitions for reference
      const formsQuery = `
        SELECT 
          form_id,
          form_name,
      ...
      const formsResult = await pool.request().query(formsQuery);
      const approvedForms = formsResult.recordset;
      ...
      const documentsQuery = `
        SELECT 
          ...
          ing.file_size,
          CASE 
            WHEN LOWER(ing.document_type) LIKE '%commercial%invoice%' OR LOWER(ing.original_filename) LIKE '%invoice%' THEN 'commercial_invoice_v1'
            WHEN LOWER(ing.document_type) LIKE '%bill%lading%' OR LOWER(ing.original_filename) LIKE '%lading%' THEN 'bill_of_lading_v1'
            WHEN LOWER(ing.document_type) LIKE '%certificate%origin%' OR LOWER(ing.original_filename) LIKE '%origin%' THEN 'certificate_origin_v1'
            WHEN LOWER(ing.document_type) LIKE '%packing%list%' OR LOWER(ing.original_filename) LIKE '%packing%' THEN 'packing_list_v1'
            WHEN LOWER(ing.document_type) LIKE '%insurance%' OR LOWER(ing.original_filename) LIKE '%insurance%' THEN 'insurance_certificate_v1'
            WHEN LOWER(ing.document_type) LIKE '%lc%' OR LOWER(ing.original_filename) LIKE '%lc%' THEN 'lc_v1'
            ELSE 'unknown'
          END as classified_form_id,
          LEN(ISNULL(ing.extracted_text, '')) as text_length
      ...
      // Initialize groups for approved forms
      approvedForms.forEach(form => {
        groupedDocuments[form.form_id] = {
          formInfo: {
            form_id: form.form_id,
            form_name: form.form_name,
      ...
      // Add unknown category for unclassified documents
      groupedDocuments['unknown'] = {
        formInfo: {
          form_id: 'unknown',
          form_name: 'Unclassified Documents',
          form_category: 'Uncategorized',
      ...
      documents.forEach(doc => {
        const formId = doc.classified_form_id;
        const targetGroup = groupedDocuments[formId] || groupedDocuments['unknown'];
        ...
        targetGroup.documents.push({
          ...
          has_extracted_text: doc.text_length > 0
        });
        
        targetGroup.count++;
        targetGroup.totalTextLength += doc.text_length;
        targetGroup.statusBreakdown[doc.status] = (targetGroup.statusBreakdown[doc.status] || 0) + 1;
      });
      
      // Remove empty groups
      Object.keys(groupedDocuments).forEach(key => {
        if (groupedDocuments[key].count === 0) {
          delete groupedDocuments[key];
        }
      });
