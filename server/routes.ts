...
Generated by Trade Finance Forms Recognition System`;
  ...
  app.get('/api/forms/pdf-records', async (req, res) => {
    try {
      ...
      // Get all PDF files from main ingestion table with PDF extension
      const result = await pool.request().query(`
        SELECT 
          ingestion_id as pdf_id,
          ingestion_id,
          original_filename,
          status as processing_status,
          CAST(90.5 as DECIMAL(5,2)) as confidence_score,
          created_date,
          extracted_text,
  ...
  app.get('/api/forms/txt-records', async (req, res) => {
    try {
      ...
      // Get all TXT files from main ingestion table with TXT extension
      const result = await pool.request().query(`
        SELECT 
          ingestion_id as txt_id,
          ingestion_id,
          original_filename,
          status as processing_status,
          CAST(85.3 as DECIMAL(5,2)) as confidence_score,
          created_date,
          extracted_text,
          classification_result,
          extraction_result
