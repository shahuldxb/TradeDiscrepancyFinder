...
Generated by Trade Finance Forms Recognition System`;
  ...
  app.get('/api/forms/definitions/:formId/fields', async (req, res) => {
    try {
      ...
      // Check if TF_Fields table exists, if not create it with proper structure
      ...
      if (tableCheck.recordset[0].count === 0) {
        console.log('TF_Fields table does not exist. Creating it...');
        await pool.request().query(`
          CREATE TABLE TF_Fields (
            id INT IDENTITY(1,1) PRIMARY KEY,
            ...
            field_type NVARCHAR(50) NOT NULL DEFAULT 'text',
            is_required BIT DEFAULT 0,
        ...
        // Insert sample field definitions for existing approved forms
        await pool.request().query(`
          INSERT INTO TF_Fields (field_id, form_id, field_name, field_label, field_type, is_required, field_order, azure_mapping, is_active)
          VALUES 
          ('commercial_invoice_number', 'commercial_invoice_v1', 'invoice_number', 'Invoice Number', 'text', 1, 1, 'InvoiceId', 1),
          ('commercial_invoice_amount', 'commercial_invoice_v1', 'total_amount', 'Total Amount', 'currency', 1, 2, 'InvoiceTotal', 1),
          ('commercial_invoice_date', 'commercial_invoice_v1', 'invoice_date', 'Invoice Date', 'date', 1, 3, 'InvoiceDate', 1),
          ('bill_of_lading_number', 'bill_of_lading_v1', 'bl_number', 'Bill of Lading Number', 'text', 1, 1, 'DocumentNumber', 1),
          ('bill_of_lading_vessel', 'bill_of_lading_v1', 'vessel_name', 'Vessel Name', 'text', 1, 2, 'VesselName', 1),
          ('bill_of_lading_port_loading', 'bill_of_lading_v1', 'port_of_loading', 'Port of Loading', 'text', 1, 3, 'PortOfLoading', 1)
        `);
        
        console.log('TF_Fields table created and populated with sample data');
      }
      
      // Now fetch the fields for the specific form
      const result = await pool.request()
        .input('formId', formId)
        .query(`
          SELECT 
            ...
            field_order, 
            ISNULL(validation_pattern, '') as validation_pattern,
            ISNULL(default_value, '') as default_value,
            ISNULL(field_options, '') as field_options,
            ISNULL(azure_mapping, '') as azure_mapping,
            ISNULL(extraction_rules, '') as extraction_rules,
      ...
      console.log(`Found ${result.recordset.length} fields for form ${formId}`);
      res.json(result.recordset);
    } catch (error) {
