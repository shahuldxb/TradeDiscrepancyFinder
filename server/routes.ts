      ...
      if (type === 'text') {
        ...
        if (pdfResult.recordset.length > 0) {
          ...
          if (filePath && fs.existsSync(filePath)) {
            ...
            // If original PDF not found, try to get text content instead
            console.log(`PDF file not found at ${filePath}, falling back to text download`);
            ...
            if (textResult.recordset.length > 0) {
              const textContent = textResult.recordset[0].content || '';
              const filename = `${ingestionId}_extracted_content.txt`;
              
              res.setHeader('Content-Type', 'text/plain');
              res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
              res.send(textContent);
            } else {
              res.status(404).json({ success: false, message: 'Neither PDF file nor extracted text found' });
            }
          }
        } else {
