  ...
  app.get('/api/forms/extracted-forms/:ingestionId', async (req, res) => {
    try {
      ...
      const fieldRecords = await pool.request()
        ...
        .query(`
          SELECT 
            field_name, field_value, confidence_score, created_date
          FROM TF_ingestion_fields 
          WHERE ingestion_id = @ingestionId
        `);
        ...
        if (pdfRecords.recordset.length > 0) {
          for (const pdf of pdfRecords.recordset) {
            ...
            forms.push({
              ...
              filename: pdf.file_path.split('/').pop() || 'Unknown',
              formType: pdf.document_type,
              confidence: 0.95,
              textContent: main.extracted_text || 'Text extraction in progress...',
              extractedFields,
          ...
          forms.push({
            ...
            confidence: 0.92,
            textContent: main.extracted_text || 'Processing complete',
            extractedFields
          });
        }
      }
