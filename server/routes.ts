...
Generated by Trade Finance Forms Recognition System`;
  ...
  app.post('/api/forms/populate-processing-tables', async (req, res) => {
    try {
      ...
      // Get TXT files from main TF_ingestion table (extracted text as TXT records)
      const txtFiles = await pool.request().query(`
        SELECT 
          ingestion_id,
          original_filename,
          extracted_text,
        ...
        AND ingestion_id NOT IN (SELECT ingestion_id FROM TF_ingestion_TXT WHERE ingestion_id IS NOT NULL)
      `);
      
      console.log(`Found ${txtFiles.recordset.length} extracted text records to populate`);
      
      // Insert extracted text into TF_ingestion_TXT table
      for (const txt of txtFiles.recordset) {
        await pool.request()
          .input('ingestion_id', txt.ingestion_id)
          .input('content', txt.extracted_text)
          .input('confidence', 95.0)
          .input('language', 'en')
          .query(`
      ...
      res.json({
        success: true,
        message: `Populated ${pdfFiles.recordset.length} PDF records and ${txtFiles.recordset.length} TXT records`,
        pdfCount: pdfFiles.recordset.length,
        txtCount: txtFiles.recordset.length
      });
    ...
    try {
      ...
      for (const data of sampleData) {
        await pool.request()
          ...
          .input('extracted_text', data.extracted_text)
          .input('extracted_data', data.extracted_data)
          .input('confidence_score', data.confidence_score)
          .query(`
            INSERT INTO TF_ingestion (
              ingestion_id, original_filename, file_path, file_size, mime_type,
              status, document_type, extracted_text, extracted_data, confidence_score, created_date
            ) VALUES (
              @ingestion_id, @original_filename, @file_path, @file_size, @mime_type,
              @status, @document_type, @extracted_text, @extracted_data, @confidence_score, GETDATE()
            )
          `);
        inserted++;
      }
