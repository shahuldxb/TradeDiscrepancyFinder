...
Generated by Trade Finance Forms Recognition System`;
  ...
  app.post('/api/forms/populate-sample-data', async (req, res) => {
    try {
      ...
      // Insert sample forms with approval workflow
      await pool.request().query(`
        ...
        VALUES ('commercial_invoice_v1', 'Commercial Invoice', 'Invoice', 'Standard commercial invoice for trade finance', 'approved', 1, 1)
      `);

      await pool.request().query(`
        IF NOT EXISTS (SELECT 1 FROM TF_forms WHERE form_id = 'bill_of_lading_v1')
        INSERT INTO TF_forms (form_id, form_name, form_category, form_description, approval_status, is_active, is_template)
        VALUES ('bill_of_lading_v1', 'Bill of Lading', 'Shipping', 'Ocean bill of lading for cargo shipment', 'pending_approval', 0, 1)
      `);

      await pool.request().query(`
        IF NOT EXISTS (SELECT 1 FROM TF_forms WHERE form_id = 'certificate_origin_v1')
        INSERT INTO TF_forms (form_id, form_name, form_category, form_description, approval_status, is_active, is_template)
        VALUES ('certificate_origin_v1', 'Certificate of Origin', 'Certification', 'Certificate of origin for goods', 'pending_approval', 0, 1)
      `);

      await pool.request().query(`
        IF NOT EXISTS (SELECT 1 FROM TF_forms WHERE form_id = 'packing_list_v1')
        INSERT INTO TF_forms (form_id, form_name, form_category, form_description, approval_status, is_active, is_template)
        VALUES ('packing_list_v1', 'Packing List', 'Logistics', 'Detailed packing list for shipment', 'pending_approval', 0, 1)
      `);
  ...
  app.get('/api/forms/definitions', async (req, res) => {
    try {
      ...
      let whereClause = '';
      if (status) {
        whereClause = `WHERE approval_status = '${status}'`;
      }
      
      const result = await pool.request().query(`
        SELECT 
          form_id, form_name, 
          ISNULL(form_category, 'General') as form_category, 
          ISNULL(form_description, '') as form_description, 
          ISNULL(form_version, '1.0') as form_version,
          ISNULL(approval_status, 'pending_approval') as approval_status, 
          approval_date, 
          ISNULL(approved_by, '') as approved_by, 
          ISNULL(approval_notes, '') as approval_notes,
          CAST(ISNULL(is_active, 0) AS BIT) as is_active, 
          CAST(ISNULL(is_template, 0) AS BIT) as is_template,
          ISNULL(processing_rules, '{}') as processing_rules,
          ISNULL(validation_rules, '{}') as validation_rules,
          ISNULL(azure_model_preference, 'prebuilt-document') as azure_model_preference,
          ISNULL(created_by, 'system') as created_by, 
          ISNULL(created_date, GETDATE()) as created_date, 
          ISNULL(updated_date, GETDATE()) as updated_date
        FROM TF_forms 
        ${whereClause}
  ...
  app.get('/api/forms/definitions/:formId/fields', async (req, res) => {
    try {
      ...
      const result = await pool.request()
        ...
        .query(`
          SELECT 
            field_id, field_name, field_label, field_type, is_required,
            field_order, validation_pattern, default_value, field_options,
            azure_mapping, extraction_rules, help_text, is_active
          FROM TF_Fields 
  ...
  app.post('/api/forms/definitions/:formId/approval', async (req, res) => {
    try {
      ...
      await pool.request()
        ...
        .input('approvedBy', approvedBy || 'back_office')
        .input('approvalNotes', notes || '')
        .input('isActive', isActive)
        .query(`
          UPDATE TF_forms 
          SET 
      ...
      res.status(500).json({ error: 'Failed to update form approval' });
    }
  });
