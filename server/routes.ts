...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Test endpoint to check ls_ table structures
  app.get('/api/lifecycle/table-structures', async (req, res) => {
    try {
      const { connectToAzureSQL } = await import('./azureSqlConnection');
      const pool = await connectToAzureSQL();
      
      const tables = ['ls_BusinessProcessWorkflows', 'ls_BusinessRules', 'ls_LifecycleStates', 'ls_DocumentExaminationStates', 'ls_LifecycleTransitionRules', 'ls_StateTransitionHistory'];
      const structures = {};
      ...
      for (const table of tables) {
        try {
          ...
          const sample = await pool.request().query(`SELECT TOP 1 * FROM ${table}`);
          
          structures[table] = {
            columns: structure.recordset,
            hasData: sample.recordset.length > 0,
      ...
      res.status(500).json({ error: 'Failed to check table structures' });
    }
  });
