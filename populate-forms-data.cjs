...
async function populateData() {
  try {
    ...
    // Create sample processing records with unique IDs
    const ingestionId1 = 'ing_sample_' + Date.now();
    ...
    await sql.query(`
      ...
      INSERT INTO TF_ingestion (
        ...
        '${ingestionId1}', 'sample_commercial_invoice.pdf', 'pdf', 1024000,
        'completed', 'Commercial Invoice\nInvoice Number: INV-001\nAmount: $5,000.00\nDate: June 17, 2025',
        'commercial_invoice', '{"invoice_number": "INV-001", "amount": "5000.00", "date": "2025-06-17"}',
    ...
    await sql.query(`
      ...
      INSERT INTO TF_ingestion (
        ...
        '${ingestionId2}', 'sample_bill_of_lading.txt', 'txt', 2048,
        'completed', 'BILL OF LADING\nB/L Number: BL-2025-001\nVessel: MV CONTAINER\nPort of Loading: Shanghai',
        'bill_of_lading', '{"bl_number": "BL-2025-001", "vessel": "MV CONTAINER", "port": "Shanghai"}',
        '[{"step": "upload", "status": "completed"}, {"step": "classification", "status": "completed"}]'
      )
    `);

    // Sample PDF processing record
    await sql.query(`
      ...
      INSERT INTO TF_ingestion_Pdf (
        ...
        'pdf_sample_001', '${ingestionId1}', 'sample_commercial_invoice.pdf', 
    ...
    await sql.query(`
      ...
      INSERT INTO TF_ingestion_TXT (
        ...
        'txt_sample_001', '${ingestionId2}', 'sample_bill_of_lading.txt',
    ...
    console.log(`Data created: ${formsCount.recordset[0].count} forms, ${pdfCount.recordset[0].count} PDF records, ${txtCount.recordset[0].count} TXT records`);
